...
nodes <- read.csv("EDM-Nodelist-3.8.csv", header = T, as.is = T)
nodes
...
V(EgoHBS[[1]])[(V(EgoHBS[[1]])$naziweight=="1")]$color <- "#E83E1A" # Mitläufer, hellrot
...
HilfsnetzwerkZwei <- delete_vertices(edm, V(edm)[category<"1"])

# Heidis Ego-Netzwerk, Stufe 1, visualisiert nach nazi-weight, nur Orgas und Kultur/Schriften und Sonstiges
EgoHBSZwei <- make_ego_graph(HilfsnetzwerkZwei, order=1, c("Heidi Benneckenstein"))

V(EgoHBSZwei[[1]])[(V(EgoHBSZwei[[1]])$naziweight=="0")]$color <- "#F7F6FF" # kein Nazi, braunrot
V(EgoHBSZwei[[1]])[(V(EgoHBSZwei[[1]])$naziweight=="1")]$color <- "#E83E1A" # Mitläufer, hellrot
V(EgoHBSZwei[[1]])[(V(EgoHBSZwei[[1]])$naziweight=="2")]$color <- "#821A0A" # Nazi, dunkelrot
V(EgoHBSZwei[[1]])[(V(EgoHBSZwei[[1]])$naziweight=="3")]$color <- "#000000" # Nazi-Größe, schwarz


class(EgoHBSZwei)
plot(EgoHBSZwei[[1]], vertex.label=NA)
